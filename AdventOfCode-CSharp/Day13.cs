...
namespace AdventOfCode_CSharp
{
    ...
    {
        private static Dictionary<string, long> _gameSurface = new Dictionary<string, long>();
        ...
        private static void ParseOutput(List<long> output)
        {
            for (var i = 0; i < output.Count; i += 3)
            {
                ...
                if (x == -1 && y == 0) Console.WriteLine($"Score: {id}");
            }
        }
    }
}