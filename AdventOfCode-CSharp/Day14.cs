...
namespace AdventOfCode_CSharp
{
    internal class Day14
    {
        private const string filePath =
            "C:\\Users\\Aaron\\source\\repos\\AdventOfCode-CSharp\\AdventOfCode-CSharp\\Resources\\Day14.txt";
        ...
        {
            var ores = 0L;
            var overSupply = new Dictionary<string, long>();

            while (Orders.Count > 0)
            {
                var NewOrders = new Dictionary<string, long>();
                foreach (var order in Orders)
                {
                    ...
                    orderAmount =
                        (long) (Math.Ceiling((double) orderAmount / (double) reactionAmount) * reactionAmount);
                    ...
                    foreach (var ingredient in reaction.Ingredients)
                        if (ingredient.Name != "ORE")
                        {
                            if (NewOrders.ContainsKey(ingredient.Name))
                                NewOrders[ingredient.Name] =
                                    NewOrders[ingredient.Name] + ingredient.Amount * factorIngredients;
                            else
        ...
        public static void ParseInput(List<string> inputs, List<Reaction> reactions)
        {
            foreach (var input in inputs)
            {
                ...
                var mainElement = new Element
                    {Amount = long.Parse(mainElementSplit[0].Trim()), Name = mainElementSplit[1].Trim()};
                ...
                foreach (var ingredient in ingredients)
                {
                    ...
                    var newIngredient = new Element
                        {Amount = long.Parse(ingredientSplit[0].Trim()), Name = ingredientSplit[1].Trim()};
